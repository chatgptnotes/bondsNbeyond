...
function VerifyMobileContent() {
  ...
  const handleVerifyOtp = async (otpValue?: string) => {
    ...
    try {
      // Get registration data from localStorage to pass during verification
      // This ensures user can be created even if server memory was cleared
      const userProfileStr = localStorage.getItem('userProfile');
      ...
      if (userProfileStr) {
        try {
          ...
          console.log('📋 [verify-mobile] Passing registration data during verification:', registrationData);
        } catch (parseError) {
          console.error('Failed to parse userProfile for verification:', parseError);
        }
      }
