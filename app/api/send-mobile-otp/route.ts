...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (useDatabaseOTP) {
      ...
      // If using hardcoded OTP, store in BOTH memory AND Supabase for reliability
      if (useHardcodedOTP && process.env.HARDCODED_OTP) {
        // Prepare temp_user_data for new registrations
        ...
        // Store in memory for later verification (used to create user account)
        ...
        // ALSO store in Supabase for cross-request reliability (memory is volatile between requests)
        ...
        try {
          ...
          console.log(`✅ [send-mobile-otp] Hardcoded OTP stored in memory AND Supabase: ${mobile}, has_temp_user_data: ${!!tempUserData}`);
        } catch (dbError) {
          console.warn(`⚠️ [send-mobile-otp] Failed to store in Supabase, using memory only:`, dbError);
        }

        return NextResponse.json({
          success: true,
          message: 'Verification code displayed on screen (testing mode)',
          devOtp: otp,
          smsStatus: 'hardcoded'
        });
      }

      // For non-hardcoded mode (Twilio not configured), store OTP in database
      const expiresAt = new Date(Date.now() + (10 * 60 * 1000)).toISOString();
