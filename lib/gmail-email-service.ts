...
const GMAIL_CONFIG = {
  from: process.env.GMAIL_FROM || 'Bonds N Beyond <chatgptnotes@gmail.com>',
...
export class GmailEmailService {
  private getEmailTemplate(type: EmailType, data: OrderData): { subject: string; html: string } {
    switch (type) {
      case 'confirmation':
        return {
          subject: `Your Bonds N Beyond NFC Order is Confirmed – ${data.orderNumber}`,
          html: orderConfirmationEmail(data)
        };
      case 'receipt':
        return {
          subject: `Receipt for Order ${data.orderNumber} | Bonds N Beyond`,
          html: receiptEmail(data)
        };
      case 'production':
        return {
          subject: `Your Card is in Production - ${data.orderNumber} | Bonds N Beyond`,
          html: inProductionEmail(data)
        };
      case 'shipped':
        return {
          subject: `Package Shipped - ${data.orderNumber} | Bonds N Beyond`,
          html: shippedEmail(data)
        };
      case 'delivered':
        return {
          subject: `Your Bonds N Beyond Card Has Arrived! - ${data.orderNumber}`,
