...
const createAdminClient = () => {
  ...
  return createClient(supabaseUrl, serviceRoleKey, {
    ...
    global: {
      // Use cross-fetch to fix Next.js 15 / Node.js 25 fetch issues
...
export interface CreateUserInput {
  ...
  password_hash?: string // Optional - will use placeholder for OTP-only registrations
}
...
export const SupabaseUserStore = {
  ...
  upsertByEmail: async (input: CreateUserInput): Promise<SupabaseUser> => {
    ...
    const newUser = {
      ...
      password_hash: input.password_hash || 'OTP_VERIFIED_NO_PASSWORD', // Placeholder for OTP-only registrations
    }
