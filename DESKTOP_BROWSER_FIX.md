...
```bash
COOKIE_DOMAIN=.bonds n beyond.ai
```

**Important Notes:**
- The leading dot (`.bonds n beyond.ai`) allows cookies to work across all subdomains
...
1. **Clear browser cookies** for bonds n beyond.ai domain
2. **Login/Register** on desktop browser
3. **Test Profile Builder** - Save profile data
4. **Test Coupon Auto-Apply** - Check if BONDS N BEYONDFM voucher applies automatically
...
- Without domain: Cookie only works on exact domain (bonds n beyond.ai)
- With `.bonds n beyond.ai`: Cookie works on all subdomains (www.bonds n beyond.ai, app.bonds n beyond.ai, etc.)
...
2. Add `COOKIE_DOMAIN=.bonds n beyond.ai` to production environment variables
